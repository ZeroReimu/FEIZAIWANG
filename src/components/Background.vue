    ...
    BackgroundFunction() {
      ...
      const update = () => {
        particles.forEach(p1 => {
          p1.x +=
            (Math.cos(p1.angle) +
              Math.cos(time * p1.oscSpeedX) * p1.oscAmplitudeX) *
            p1.speed;
          p1.y +=
            (Math.sin(p1.angle) +
              Math.cos(time * p1.oscSpeedY) * p1.oscAmplitudeY) *
            p1.speed;
        ...
        particles.map(p1 => {
          particles
            ...
            .map(p2 => {
              ...
              p1.speed = lerp(
                p1.speed,
                p1.speed + (0.05 / p1.connectDistance) * dist,
                0.2
              );
              return {
                p1,
                ...
                opacity:
                  Math.floor(
                    (100 / p1.connectDistance) * (p1.connectDistance - dist)
                  ) / 100
              };
            })
            .forEach((line, i) => {
